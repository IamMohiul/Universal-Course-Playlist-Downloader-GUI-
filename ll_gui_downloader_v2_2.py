#!/usr/bin/env python3
"""
Universal GUI Downloader v2.2
- Fixes folder/serial naming: uses playlist index with zero-padding.
- Removes "NA - NA" by setting outtmpl_na_placeholder to empty.
- Green progress bars, speed/ETA, remaining items, cancel support.
"""
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Universal GUI Downloader (yt-dlp) v2.2")
        ...
        style.configure("green.Horizontal.TProgressbar",
                        ...
                        background="#22c55e", lightcolor="#22c55e", darkcolor="#22c55e")
        ...
        ttk.Label(self, text="cookies.txt:").grid(row=1, column=0, sticky="w", **pad)
        ttk.Entry(self, textvariable=self.cookies_var).grid(row=1, column=1, columnspan=3, sticky="we", **pad)
        ...
        for c in (1,2,3): self.grid_columnconfigure(c, weight=1)
    ...
    def start_download_thread(self):
        ...
        if not url:
            self._log("Please paste a URL."); self.status_var.set("Need URL"); return
        self.stop_flag = False
        self.btn_start.config(state=tk.DISABLED); self.btn_cancel.config(state=tk.NORMAL)
        self.status_var.set("Analyzing…"); self._log("Starting download…")
        threading.Thread(target=self._worker, args=(url,), daemon=True).start()

    def cancel_download(self):
        self.stop_flag = True
        self._log("Cancel requested. Finishing current fragment…"); self.status_var.set("Cancelling…")

    def _poll_queue(self):
        try:
            while True:
                msg = self.q.get_nowait(); kind = msg.get("kind")
                if kind == "status":
                    ...
                    self.pb_overall["value"] = 0; self.overall_pct_var.set("0%")
                elif kind == "progress":
                    file_pct = int(msg.get("file_frac",0)*100); overall_pct = int(msg.get("overall_frac",0)*100)
                    self.pb_file["value"] = file_pct; self.file_pct_var.set(f"{file_pct}%")
                    self.pb_overall["value"] = overall_pct; self.overall_pct_var.set(f"{overall_pct}%")
                    self.file_title_var.set(msg.get("title","—")[:90])
                    self.speed_var.set(msg.get("speed","0 B/s")); self.eta_var.set(msg.get("eta","--:--"))
                    self.remain_var.set(str(msg.get("remaining", 0)))
                elif kind == "done":
                    self.btn_start.config(state=tk.NORMAL); self.btn_cancel.config(state=tk.DISABLED)
    ...
    def _ydl_opts(self, outdir: Path, cookies: Path | None):
        return {
            # Robust output template: 3-digit serial using playlist_index
            "outtmpl": str(outdir / "%(playlist_title)s/%(playlist_index)03d - %(title)s.%(ext)s"),
            # Remove "NA" placeholders entirely
    ...
    def _extract_entries(self, url, opts):
        ...
        if not info: raise RuntimeError("Failed to extract metadata; check the URL or cookies.")
    ...
    def _make_hook(self, idx, total, title):
        def hook(d):
            if self.stop_flag: raise KeyboardInterrupt("User cancelled.")
            status = d.get("status")
            if status == "downloading":
                downloaded = d.get("downloaded_bytes") or 0
                total_bytes = d.get("total_bytes") or d.get("total_bytes_estimate") or 0
                if total_bytes: file_frac = downloaded / total_bytes
                else:
                    fi = d.get("fragment_index") or 0; fc = d.get("fragment_count") or 0
                    file_frac = (fi / fc) if fc else 0.0
                overall_frac = ((idx - 1) + file_frac) / total if total else file_frac
                speed = d.get("speed"); eta = d.get("eta")
                self.q.put({"kind":"progress","title":title,
                ...
                self.q.put({"kind":"progress","title":title,"file_frac":1.0,
                            "overall_frac":overall_frac,"speed":"—","eta":"00:00",
                            "remaining": max(0, total - idx)})
        return hook
    ...
    def _worker(self, url: str):
        ...
        try:
            ...
            for idx, entry in enumerate(entries, 1):
                if self.stop_flag: cancelled = True; break
                if not entry: continue
                title = entry.get("title") or entry.get("id") or f"item {idx}"
                self._log(f"[{idx}/{total}] {title}")
                hook = self._make_hook(idx, total, title)
                run_opts = dict(opts); run_opts["progress_hooks"] = [hook]
